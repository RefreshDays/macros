...
def sanitize_ocr_line(line: str) -> str:
    ...
    cleaned = re.sub(r"[^가-힣0-9% ]+$", "", cleaned)
    cleaned = re.sub(r"수\s*수\s*네\s*료", "수수료", cleaned)
    cleaned = re.sub(r"수\s*수\s*내\s*료", "수수료", cleaned)
...
class MacroApp:
    def __init__(self):
        self.root = tk.Tk()
        self.root.title("설비 수주 자동응답 매크로 v1.9.5")
    ...
    def _set_hangul_ime():
        try:
            ...
            ctypes.windll.user32.SendMessageW(ime_hwnd, 0x0283, 0x0001, 0x0000_0412)
        except Exception:
            pass
    ...
    def _build_ui(self):
        ...
        self.entry_room.bind("<FocusIn>", lambda e: self._set_hangul_ime())
        self.entry_response = None

        ttk.Label(cfg, text="응답 문구:").grid(row=1, column=0, sticky=tk.W, pady=2)
        if self.entry_response is None:
    ...
    def _extract_focus_lines(self, positioned_lines):
        ...
        tail_lines = ordered[-16:]
        max_y = tail_lines[-1]["y"]
        min_y = tail_lines[0]["y"]
        threshold = min_y + int((max_y - min_y) * 0.25)
    ...
    def _monitor_loop(self):
        ...
        while self.running:
            try:
                ...
                # ── 기준선 설정 (전체 화면에서 기존 메시지 등록) ────────────────
                if not baseline_set:
                    baseline_set = True
                    all_lines_as_focus = [{"text": item["text"], "y": item["y"]} for item in positioned_lines]
                    baseline_candidates = self._extract_all_candidates(
                        all_lines_as_focus, current_region_names, response_text
                    )
                ...
                # 확정 임계값 도달 후보 (가장 먼저 등장한 것 1개만 처리)
                ready = [
                    (info["first_seen"], dk, info["line"], info["parsed"])
