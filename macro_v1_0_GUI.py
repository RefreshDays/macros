# -*- coding: utf-8 -*-
"""
설비 수주 자동응답 매크로 macro_v1_0 GUI
- 단일 실행 파일
- 후보 확정 2회
- 제한 지역 데이터 포함
- 시스템 시간(초 단위) 기반 로그
- macro_v1_0 수정: 중복 응답 완전 차단
...
QUEUE_TTL_SECONDS = 6.0  # macro_v1_0: 2.0 → 6.0 (TTL 만료 타이밍 재큐잉 방지)
MAX_QUEUE_SIZE = 10
...
# macro_v1_0: 수수료 OCR 오인식 계열 쌍 (1↔10, 2↔20, 3↔30)
...
class MacroApp:
    def __init__(self):
        self.root = tk.Tk()
        self.root.title("설비 수주 자동응답 매크로 macro_v1_0 GUI")
    ...
    def _mark_related_commissions_processed(self, region: str, job: str, commission: int):
        """macro_v1_0: 전송 후 동일 지역+작업의 오인식 수수료 계열도 즉시 차단한다.
    ...
    def _monitor_loop(self):
        ...
        while self.running:
            try:
                ...
                # macro_v1_0: 전송 직전 선행 등록(pre-mark) — 어떤 경로로도 2회 전송 불가
                self.processed_keys.add(dup_key)
