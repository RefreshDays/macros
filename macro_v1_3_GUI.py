# -*- coding: utf-8 -*-
"""
설비 수주 자동응답 매크로 macro_v1_3 GUI
- 단일 실행 파일
- 후보 확정 2회
- 제한 지역 데이터 포함
- 시스템 시간(초 단위) 기반 로그
- macro_v1_1 수정: 중복 응답 완전 차단
  1. 전송 직전 processed_keys 선행 등록 (pre-mark)
  2. 수수료 OCR 오인식 계열(1↔10, 2↔20, 3↔30) 동시 차단
  3. QUEUE_TTL 2.0s → 6.0s 확장 (TTL 만료 타이밍 재큐잉 방지)
- macro_v1_2 수정 (배포 버전):
  1. 중지 버튼 시 큐/대기 후보 즉시 비움 + 인터럽트 sleep + running 재확인
  2. fuzzy 매칭 대상 최소 3글자 이상으로 제한 (2글자 지역 오인식 방지)
  3. 동일 (지역+작업) 쌍 전송 후 30초 유예(grace) 적용 — 수수료 OCR 오독 재응답 차단
- macro_v1_3 수정:
  1. 허용패턴 추가: 수10, 수 10, 수10%, 수 10%
  2. 인식 키워드 변경: 싱막, 하막, 역류, 고압 4개
"""
...
def parse_message(line: str, selected_regions: List[str], response_text: str = "네") -> Optional[Tuple[str, str, int]]:
    ...
    for variant in selected_variants:
        ...
        if compact_variant in compact_line or (len(compact_variant_norm) >= 2 and compact_variant_norm in compact_line):
    ...
    job_keywords = {"싱막", "하막", "역류", "고압", "성막", "생막", "씽막"}
    region_candidates = [
        candidate
...
class MacroApp:
    def __init__(self):
        self.root = tk.Tk()
        self.root.title("설비 수주 자동응답 매크로 macro_v1_3 GUI")
        ...
        self.sent_group_keys: Dict[str, float] = {}  # macro_v1_2: group_key → 마지막 전송 시각
    ...
    def _extract_focus_lines(self, positioned_lines):
        ...
        # 마지막 메시지 누락을 줄이기 위해 하단 recent line을 넉넉하게 그대로 본다.
        return ordered[-18:]
    ...
    def _extract_all_candidates(self, focus_lines, region_names: List[str], response_text: str) -> List[Tuple[float, str, str, tuple, bool]]:
        ...
        for item in focus_lines:
            ...
            candidates.append((item["y"], dup_key, display_line, parsed, strong))
        ...
        while self.running:
            try:
                ...
                for y, dup_key, line, parsed, strong in all_candidates:
                    if dup_key in self.processed_keys:
                        continue
                    # macro_v1_2: 동일 (지역+작업) 쌍 전송 후 grace 기간 내 재진입 차단
                    _gk = self._candidate_group_key(parsed)
                    if _gk in self.sent_group_keys:
                        if now - self.sent_group_keys[_gk] < SENT_GROUP_GRACE_SECONDS:
                            self.processed_keys.add(dup_key)
                            continue
                        else:
                            del self.sent_group_keys[_gk]
                ...
                if self._send_response(response_text):
                    ...
                    # macro_v1_2: 동일 (지역+작업) 쌍 재전송 차단용 그룹 시각 기록
                    self.sent_group_keys[self._candidate_group_key(parsed)] = self.last_send_time
